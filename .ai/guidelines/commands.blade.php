...
- Any time you need to run an npm command, use `lando npm` instead of running `npm` directly on the host. Example, if you
  wanted to install a dependency you would run `lando npm install package-name` instead of `npm install package-name`.

### Running other container

- Any time you need to run a command inside the container, use `lando ssh -c "command to run"` instead of running it on
